...
import { Upload, Download, X, Scissors, AlertCircle, Sparkles } from 'lucide-react';
...
export default function BackgroundRemover() {
  ...
  const reset = () => { setSelectedFile(null); setResultImage(null); setError(null); };

  return (
    <div className="space-y-8">
      <div className="text-center space-y-4"><div className="w-16 h-16 bg-tertiary/10 rounded-2xl flex items-center justify-center mx-auto"><Scissors className="w-8 h-8 text-tertiary" /></div>
        <h1 className="text-3xl font-display font-black uppercase tracking-tighter text-ink">Background Remover</h1><p className="text-ink/60">AI-powered background removal</p></div>
      ...
      {selectedFile && (<div className="space-y-8">
        ...
        {resultImage && (<div className="p-6 bg-slate-800/50 rounded-2xl"><h3 className="font-bold text-ink uppercase tracking-widest text-sm mb-4">Result</h3><div className="relative rounded-xl overflow-hidden border border-slate-700"><img src={resultImage} alt="Result" className="w-full h-64 object-contain bg-slate-800" /></div></div>)}
        <div className="flex gap-4"><button onClick={removeBackground} disabled={isProcessing} className="flex-1 py-4 bg-tertiary text-black font-bold uppercase tracking-widest rounded-xl hover:bg-tertiary/90 transition-colors disabled:opacity-50 flex items-center justify-center gap-2">{isProcessing ? <><Sparkles className="w-5 h-5 animate-pulse" /> Processing...</> : <><Sparkles className="w-5 h-5" /> Remove Background</>}</button>
